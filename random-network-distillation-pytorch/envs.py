...
max_step_per_episode = config.MaxStepPerEpisode
...
class MaxAndSkipEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i in range(self._skip):
            obs, reward, done, info = step_env(self.env,action)
            if self.is_render:
                self.env.render()
            if i == self._skip - 2:
...
class MontezumaInfoWrapper(gym.Wrapper):
    ...
    def step(self, action):
        obs, rew, done, info = step_env(self.env,action)
...
class AtariEnvironment(Environment):
    ...
    def run(self):
        ...
        while True:
            ...
            s, reward, done, info = step_env(self.env,action)
...
def step_env(env, action):
    ...
    if len(result) == 5:
        ...
        raise ValueError(f"Unexpected number of values returned: {len(result)}")
