...
from my_maddpg.environment.mpe import MPE_Env
from my_maddpg.utils.utils import space2shape
from my_maddpg.environment.mpe import MultiAgentEnvWrapper
...
class DummyVecMultiAgentEnv:  
    ...
    def __init__(self, env_fns, env_seed):
        self.waiting = False
        self.closed = False
        self.envs = [fn(env_seed=env_seed + inx_env)
                     for inx_env, fn in enumerate(env_fns)]
        env = self.envs[0]
        ...
        self.buf_state = [np.zeros(space2shape(self.state_space))
...
def make_envs(config: Namespace):
    def make(env_seed=None):
        ...
        return MultiAgentEnvWrapper(MPE_Env(config))
