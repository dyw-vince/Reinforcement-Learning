...
discrete_actions = [
    np.array([0.0, 0.04, 0.0]),    # 前进
    np.array([-0.15, 0.01, 0.0]),   # 向左前进
    np.array([0.15, 0.01, 0.0]),    # 向右前进
    np.array([-0.30, 0.005, 0.0]),   # 向左前进
    np.array([0.30, 0.005, 0.0]),    # 向右前进
    # np.array([-0.15, 0.01, 0.1]),     # 刹车
    # np.array([0.15, 0.01, 0.1])     # 刹车
]
...
class CarV2SkipFrame(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i in range(self.skip):
            ...
            reward =-0.5 if edge else reward
    ...
    def edge(self,obs):
        s = obs[:84, 6:90, :]
        #取第76行的13个像素的前两个点和后两个点的绿色通道，如果有一个绿色说明在边界了
...
class DQNAgent:
    def __init__(self,config):
        ...
        # self.eval_env=PreprocessObservation(CarV2SkipFrame(eval_env, skip=SKIP_N), shape=84)
...
def train(config,env,agent):
    ...
    for episode in range(config["episode"]):
        ...
        if episode >= config['warmup_episodes']:
            ...
            print(f"Episode: {episode}, Train Reward: {total_reward:4f}, This Eval Avg Reward:{avg_reward:4f},  Best Eval Avg Reward: {agent.best_avg_reward:4f}")
    return rewards_list
def eval_and_record(agent):
    ...
    test_env.close()
def plot_rewards(reward_list, window=10,save_path='./reward_plot.png'):
    plt.figure(figsize=(12, 6))
    plt.plot(reward_list, label='Episode Reward')
    
    # 平滑曲线（滑动平均）
    if len(reward_list) >= window:
        moving_avg = [sum(reward_list[i-window:i])/window for i in range(window, len(reward_list)+1)]
        plt.plot(range(window-1, len(reward_list)), moving_avg, label=f'{window}-Episode Moving Avg', color='orange')
    
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('DQN Training Performance on CarRacing-v3')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    # 保存图像
...
reward_list=train(config,env,agent)
