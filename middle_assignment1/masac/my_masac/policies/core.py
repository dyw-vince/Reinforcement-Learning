...
class ActivatedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def activated_rsample_and_logprob(self):
        ...
        correction = - 2. * (torch.log(Tensor([2.0])).to(
            self.device) - act_pre_activated - softplus(-2. * act_pre_activated))
        log_prob += correction
...
#     # def activated_rsample(self):
#     #     return self.activation_fn(self.distribution.rsample())

#     def activated_rsample_and_logprob(self):
#         act_pre_activated = self.distribution.sample()  # sample without being activated.
#         act_activated = self.activation_fn(act_pre_activated)
#         log_prob = self.distribution.log_prob(act_pre_activated)
...
class ActorNet_SAC(Module):
    def __init__(self,
        ...
        self.out_log_std = nn.Linear(
            hidden_sizes[-1], action_dim, device=device)
        self.dist = ActivatedDiagGaussianDistribution(
            action_dim, activation_action, device)
