...
from my_masac.environment import make_envs
from my_masac.utils.utils import combine_actions
from my_masac.utils.utils import set_seed
from my_masac.agents.masac_agents import MASAC_Agents
...
def get_arguments(method, env, env_id, config_path=None, parser_args=None, is_test=False):
    ...
    """
    config_path = "my_masac/configs/config.yaml"
...
class Runner(object):
    def __init__(self, configs):
        ...
        for group in range(self.num_groups):
            ...
            self.agents.append(MASAC_Agents(self.configs[group], _env))
        ...
        self.adversary_scores = defaultdict(lambda: deque(maxlen=100))
        self.agent_scores = defaultdict(lambda: deque(maxlen=100))
            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    for j, agent in enumerate(self.agents):
                        episode_score = np.sum(itemgetter(
                            *agent.agent_keys)(info[i]["episode_score"]))

                        if j == 0:
                            self.adversary_scores[i].append(
                                episode_score)
                            adversary_score_list.append(episode_score)
                            avg_score = np.mean(self.adversary_scores[i])
                        else:
                            self.agent_scores[i].append(episode_score)
                            agent_score_list.append(episode_score)
                            avg_score = np.mean(self.agent_scores[i])

                        if self.use_wandb:
                            step_info[f"Train-Results/Episode-Rewards-Group{j}/rank_{self.rank}/env-%d" %
                                      i] = episode_score
                            step_info[f"Train-Results/Episode-Rewards-Avg-Group{j}/rank_{self.rank}/env-%d" %
                                      i] = avg_score
                        agent.log_infos(step_info, self.current_step)

                    total_score = adversary_score_list[-1]+agent_score_list[-1]
                    total_avg_score = (np.mean(
                        self.adversary_scores[i])+np.mean(self.agent_scores[i]))/2
                    if self.use_wandb:
                        total_info[f"Train-Results/Episode-Rewards-Total/rank_{self.rank}/env-%d" %
                                   i] = total_score
                        total_info[f"Train-Results/Episode-Rewards-Avg-Total/rank_{self.rank}/env-%d" %
                                   i] = total_avg_score
                    self.agents[0].log_infos(total_info, self.current_step)
    ...
    def test(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
                    ...
                    for i_group in range(self.num_groups):
                        ...
                        if test_mode:
                            ...
                            if self.configs[i_group].test_mode:
                                print("Group: %d, Episode: %d, Score: %.2f" %
                                      (i_group, episode_count, episode_score))
                        else:
                            for agent in self.agents:
                                episode_score = np.sum(itemgetter(
                                    *agent.agent_keys)(info[i]["episode_score"]))
        ...
        for i_group in range(self.num_groups):
            ...
            if config.render_mode == "rgb_array" and config.render:
                ...
                videos_info = {f"Videos_Test_Group_{i_group}": np.array([episode_videos[i_group]],
