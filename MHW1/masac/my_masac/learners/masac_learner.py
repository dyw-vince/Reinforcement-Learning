...
class MASAC_Learner():
    def __init__(self,
        ...
        if self.use_automatic_entropy_tuning:
            self.target_entropy = {
                key: -policy.action_space[key].shape[-1] for key in self.model_keys}
    ...
    def update(self, sample):
        ...
        # train the model
        _, actions_eval, log_pi_eval = self.policy(
            observation=obs, agent_ids=IDs)
        _, actions_next, log_pi_next = self.policy(
            observation=obs_next, agent_ids=IDs)
        ...
        for key in self.model_keys:
            ...
            target_value = target_q[key].reshape(
                bs) - self.alpha[key] * log_pi_next_eval
            backup = rewards[key] + (1 - terminals[key]) * \
                self.gamma * target_value
            td_error_1, td_error_2 = action_q_1_i - backup.detach(), action_q_2_i - \
                backup.detach()
            td_error_1 *= mask_values
            td_error_2 *= mask_values
            loss_c = ((td_error_1 ** 2).sum() +
                      (td_error_2 ** 2).sum()) / mask_values.sum()
            ...
            if self.use_parameter_sharing:
                actions_eval_joint = actions_eval[key].reshape(
                    batch_size, self.n_agents, -1).reshape(batch_size, -1)
            else:
                actions_eval_detach_others = {k: actions_eval[k] if k == key else actions_eval[k].detach()
                                              for k in self.model_keys}
                actions_eval_joint = self.get_joint_input(
                    actions_eval_detach_others, (batch_size, -1))
            ...
            loss_a = ((self.alpha[key] * log_pi_eval_i - policy_q)
                      * mask_values).sum() / mask_values.sum()
            ...
            if self.use_automatic_entropy_tuning:
                alpha_loss = - \
                    (self.log_alpha[key] * (log_pi_eval_i +
                     self.target_entropy[key]).detach()).mean()
    ...
    def load_model(self, path, model=None):
        ...
        if model is not None:
            ...
            path = os.path.join(path, seed+self.config.run_name)
