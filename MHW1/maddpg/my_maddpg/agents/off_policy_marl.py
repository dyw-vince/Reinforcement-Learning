...
from my_maddpg.utils.utils import get_time_string, create_directory, space2shape
from my_maddpg.buffer.buffer import MARL_OffPolicyBuffer
from my_maddpg.learners.maddpg_learner import MADDPG_learner
...
class OffPolicyMARLAgents:  # (MARLAgents)
    ...
    def __init__(self,
        ...
        self.start_training = config.start_training if hasattr(
            config, "start_training") else 1

        self.training_frequency = config.training_frequency if hasattr(
            config, "training_frequency") else 1
        ...
        self.use_global_state = config.use_global_state if hasattr(
            config, "use_global_state") else False

        self.world_size = 1
        # Environment attributes.
        self.state_space = envs.state_space if self.use_global_state else None
        self.episode_length = config.episode_length if hasattr(
            config, "episode_length") else envs.max_episode_steps
        ...
        self.model_dir_save = os.path.join(
            os.getcwd(), config.model_dir, seed + config.run_name)
        config_dict = vars(config)

        # predefine necessary components
        self.model_keys = [self.agent_keys[0]
                           ] if self.use_parameter_sharing else self.agent_keys
        self.policy = None
        self.learner = None
        self.memory = None

        self.on_policy = False
        self.start_greedy = config.start_greedy if hasattr(
            config, "start_greedy") else None
        self.end_greedy = config.end_greedy if hasattr(
            config, "start_greedy") else None
        self.delta_egreedy = None
        self.e_greedy = None

        self.start_noise = config.start_noise if hasattr(
            config, "start_noise") else None
        self.end_noise = config.end_noise if hasattr(
            config, "end_noise") else None
        self.delta_noise = None
        self.noise_scale = None

        self.actions_low = self.action_space.low if hasattr(
            self.action_space, "low") else None
        self.actions_high = self.action_space.high if hasattr(
            self.action_space, "high") else None
    ...
    def _build_representation(self, representation_key,
                              input_space,
                              config):
        """
        ...
        for key in self.model_keys:
            ...
            input_representations = dict(
                ...
                device=self.device)
            representation[key] = Basic_Identical(**input_representations)
        return representation

    def _build_learner(self, *args):
        return MADDPG_learner(*args)
    ...
    def _build_inputs(self,
        ...
        agents_id = None
        obs_input = {k: np.stack([data[k] for data in obs_dict]).reshape(
            bs, -1) for k in self.agent_keys}
    ...
    def exploration(self, batch_size,
        ...
        if self.e_greedy is not None:
            ...
            if self.use_parameter_sharing:
                ...
                pi_actions_dict[key] += np.random.normal(
                    0, self.noise_scale, size=pi_actions_dict[key].shape)
            else:
    ...
    def train(self, n_steps):
        ...
        if self.use_rnn:
            with tqdm(total=n_steps) as process_bar:
                step_start, step_last = deepcopy(
                    self.current_step), deepcopy(self.current_step)
                ...
                while step_last - step_start < n_steps_all:
                    ...
                    process_bar.update(
                        (self.current_step - step_last) // self.n_envs)
        ...
        for _ in tqdm(range(n_steps)):
            ...
            policy_out = self.action(
                obs_dict=obs_dict, avail_actions_dict=avail_actions, test_mode=False)
    ...
    def run_episodes(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
        ...
        if test_mode:
            if self.config.render_mode == "rgb_array" and self.render:
                ...
                videos_info = {"Videos_Test": np.array(
                    [episode_videos], dtype=np.uint8).transpose((0, 1, 4, 2, 3))}
                self.log_videos(info=videos_info, fps=self.fps,
                                x_index=self.current_step)
    ...
    def test(self, env_fn, n_episodes):
        ...
        scores = self.run_episodes(
            env_fn=env_fn, n_episodes=n_episodes, test_mode=True)
        return scores
    ...
    def log_videos(self, info: dict, fps: int, x_index: int = 0):
        ...
        save_dir = os.path.join(self.config.video_dir,
        ...
        for k, v in info.items():
            ...
            # 保存到本地
            gif_path = os.path.join(save_dir, k+".gif")
            frames = v[0].transpose(0, 2, 3, 1).astype("uint8")
