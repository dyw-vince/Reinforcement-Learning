...
class QMIX_Agents:
    def __init__(self,config):
        ...
        self.current_Q_MIX=Q_MIX_Net(config).to(self.device)
        self.target_Q_MIX=Q_MIX_Net(config).to(self.device)
    ...
    def train(self, buffer):
        ...
        # loss = torch.sum((q_total_current - targets.detach())** 2)/self.batch_size
